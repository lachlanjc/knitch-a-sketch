...
const PatternCarousel = ({
  ...
  <div className="h-[80vh] overflow-hidden">
    <Carousel orientation="vertical" className="h-full">
      <CarouselContent className="px-6 py-6 -my-6">
        {Children.toArray(children).map((child, index) => (
          <CarouselItem
            className="basis-auto"
            key={(child as { key?: string }).key ?? `piece-${index}`}
          >
            {child}
          </CarouselItem>
        ))}
...
const PieceCard = ({
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-4 text-zinc-900 shadow-xl">
